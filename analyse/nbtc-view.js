...
(function () {
  ...
  var SAMPLE = [
    ...
    "GUBBINS, DEAN\t01/01/2000\tGBR\t574444444 (GBR)\tBA430\t05/09/2025\tLHR\tAMS\tDC",
    "GUBBINS, DEAN\t01/01/2000\tGBR\t574444444 (GBR)\tBA429\t08/09/2025\tAMS\tLHR\tDC",
    "GUBBINS, DEAN\t01/01/2000\tGBR\t574444444 (GBR)\tU27653\t11/10/2025\tLGW\tAMS\tDC",
    "GUBBINS, DEAN\t01/01/2000\tGBR\t574444444 (GBR)\tU27654\t14/10/2025\tAMS\tLGW\tDC",
    "GUBBINS, DEAN\t01/01/2000\tGBR\t574444444 (GBR)\tFR8542\t15/11/2025\tSTN\tAGP\tDC",
    "GUBBINS, DEAN\t01/01/2000\tGBR\t574444444 (GBR)\tFR8543\t17/11/2025\tAGP\tSTN\tDC",
    ...
    "GUBBINS, DEAN\t01/01/2000\tGBR\t574444444 (GBR)\tKL1009\t03/01/2026\tAMS\tLHR\tDC",
    ...
    "GUBBINS, DEAN\t01/01/2000\tGBR\t575555555 (GBR)\tFR8543\t24/01/2026\tAGP\tSTN\tDC",
    "GUBBINS, DEAN\t01/01/2000\tGBR\t575555555 (GBR)\tBA306\t07/02/2026\tLHR\tAMS\tDC",
    "GUBBINS, DEAN\t01/01/2000\tGBR\t575555555 (GBR)\tBA307\t07/02/2026\tAMS\tLHR\tDC",
    "GUBBINS, DEAN\t01/01/2000\tGBR\t575555555 (GBR)\tVY7821\t14/02/2026\tLTN\tAGP\tDC",
    "GUBBINS, DEAN\t01/01/2000\tGBR\t575555555 (GBR)\tVY7822\t15/02/2026\tAGP\tLTN\tDC",
    "GUBBINS, DEAN\t01/01/2000\tGBR\t575555555 (GBR)\tEI155\t21/02/2026\tLHR\tDUB\tDC",
    "GUBBINS, DEAN\t01/01/2000\tGBR\t575555555 (GBR)\tEI154\t21/02/2026\tDUB\tLHR\tDC",
    "GUBBINS, DEAN\t01/01/2000\tGBR\t575555555 (GBR)\tAF1281\t28/02/2026\tLHR\tCDG\tDC",
    "GUBBINS, DEAN\t01/01/2000\tGBR\t575555555 (GBR)\tAF1580\t28/02/2026\tCDG\tLHR\tDC"
  ].join("\n");
  ...
  function mount(host){
    ...
    var find = el("div", "nb-findings"); find.id = "nb-findings"; panel.appendChild(find);
  ...
  function analyse(){
    ...
    if (!recs.length) { pane("map").innerHTML = ""; pane("map").appendChild(el("div", "nb-empty", "No passenger rows found. Check the columns.")); state.journeys = null; renderFindings(null); if (meta) meta.textContent = ""; return; }
    ...
    state.findings = (N && N.findings) ? N.findings(recs) : null; renderFindings(state.findings);
  ...
  function drawMap(j){
    ...
    try {
      ...
      var AIRICON = (window.SolarEntityStyle && window.SolarEntityStyle.mapIcon) ? window.SolarEntityStyle.mapIcon("location", 26) : null;
      Object.keys(j.airports).forEach(function(k){ var a = j.airports[k]; var mk;
        if (AIRICON) mk = L.marker([a.lat, a.lon], { icon: AIRICON });
  ...
  /* Findings: the briefing headline. Factual, counted signals (same-day
   * turnarounds, no-shows, passport switch, co-travellers, tightening cadence,
   * spread of departure airports) shown as toned chips above the tabs, so the
   * "so what" is the first thing an analyst reads. */
  function renderFindings(f){
    var host = doc.getElementById("nb-findings"); if (!host) return; host.innerHTML = "";
    if (!f || !f.signals || !f.signals.length){ host.className = "nb-findings"; return; }
    host.className = "nb-findings nb-findings-on";
    var head = el("div", "nb-findings-h");
    head.appendChild(el("span", "nb-findings-t", "Findings"));
    if (f.subject) head.appendChild(el("span", "nb-findings-s", f.subject.label));
    host.appendChild(head);
    var row = el("div", "nb-findings-row");
    f.signals.forEach(function(sg){
      var chip = el("div", "nb-find nb-find-" + sg.tone);
      var ig = ico(sg.icon, 15); if (ig) chip.appendChild(ig);
      var tx = el("div", "nb-find-tx");
      tx.appendChild(el("div", "nb-find-l", sg.label));
      tx.appendChild(el("div", "nb-find-d", sg.detail));
      chip.appendChild(tx); row.appendChild(chip);
    });
  ...
  function drawTimeline(j, trips){
    ...
    var track = el("div", "nb-track"); track.style.minWidth = Math.max(600, tl.length * 108) + "px";
    ...
    tl.forEach(function(tr){
      ...
      var _tx = pct(dms(tr.out.date));
      var g = el("div", "nb-trip " + (lane ? "nb-trip-lo" : "nb-trip-hi") + (_tx > 86 ? " nb-trip-r" : "")); g.style.left = _tx + "%";
    ...
    var scroll = el("div", "nb-trackscroll"); scroll.appendChild(track); host.appendChild(scroll);
  ...
  function injectStyle(){
    ...
    var css = [
      ...
      ".nb-findings{margin:8px 14px 0;display:none}.nb-findings-on{display:block}.nb-findings-h{display:flex;align-items:baseline;gap:9px;margin-bottom:6px}.nb-findings-t{font-size:var(--fs-xs);text-transform:uppercase;letter-spacing:.09em;color:var(--faint)}.nb-findings-s{font-size:var(--fs-sm);color:var(--text);font-weight:600}",
      ".nb-findings-row{display:flex;gap:8px;flex-wrap:wrap}.nb-find{display:flex;gap:8px;align-items:flex-start;background:var(--panel-2);border:1px solid var(--line);border-left-width:3px;border-radius:8px;padding:6px 10px;max-width:340px}.nb-find .ent-ico{margin-top:1px}.nb-find-l{font-size:var(--fs-sm);color:var(--text);font-weight:600}.nb-find-d{font-size:var(--fs-xs);color:var(--dim);margin-top:1px;line-height:1.35}.nb-find-alert{border-left-color:#d86a6a}.nb-find-note{border-left-color:#e8a13a}.nb-find-info{border-left-color:#8ea2ff}",
      ...
      ".nb-trackscroll{overflow-x:auto;overflow-y:hidden;margin-bottom:8px}",
      ".nb-track{position:relative;height:158px;background:var(--panel-2);border:1px solid var(--line);border-radius:var(--radius);overflow:hidden}",
      ...
      ".nb-tchip{position:absolute;left:-5px;min-width:92px;background:var(--panel-3,#141d2b);border:1px solid var(--line);border-left:3px solid #8ea2ff;border-radius:6px;padding:3px 7px;z-index:3;white-space:nowrap}.nb-trip-hi .nb-tchip{top:6px}.nb-trip-lo .nb-tchip{top:98px}.nb-tchip-ab{border-color:#e8a13a}.nb-trip-r .nb-tchip{left:auto;right:-5px}",
